...
export const createDefaultPermissions = async () => {
  ...
  if (count > 0) {
    console.log(
      `ℹ️ Permissions jadvalida allaqachon ${count} ta yozuv mavjud. Yangi defaultlar qo‘shilmadi.`
    );
    return;
  }

  // Transaction bilan bulk yaratish
  await Permission.sequelize!.transaction(async (transaction) => {
  ...
  console.log(
    `✅ ${defaultPermissions.length} ta default permission muvaffaqiyatli yaratildi.`
  );
};
